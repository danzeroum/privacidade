...
| Risco-005 | Governança de mudança rompida: upload manual reverteu PR revisado, a main estava vermelha e nenhum workflow era required status check | P1 | Lacuna real | **Fechado** | `2cf1c18` `f86020e` `e1a35df` `77afe42` · sub-item (d) fechado: schema e invariantes entram no CI pelo check `Constraints e invariantes do banco`, e o contrato passa a ser lido pelo teste `PR 16 · invariante de paridade — contrato e mock, nos dois sentidos (Risco-034)`. Sub-item (c) fechado em `e1a35df`: workflow `Relógio do programa` roda diariamente as quatro varreduras de prazo e abre alerta idempotente, provado por `PR 29 · Risco-005(c) — o relógio do programa dispara sem sessão aberta`. Sub-item (b) fechado em `77afe42`: três jobs nomeados em `Segurança` — `Segredos no diff e no histórico` (gitleaks sobre a árvore e o histórico completo), `Dependências vulneráveis` (limiar `high` com exceção datada) e `CodeQL`. Provado por `PR 30 · Risco-005(b) — segredo, dependência e código sob varredura`. **Nota:** o CodeQL **informa e não bloqueia** — o `analyze` falha por erro de varredura, não por achado, e marcá-lo como *required* não impede merge de código com alerta aberto | Main restaurada (#19), T11 entregue (#18), proteção de branch evidenciada (#21–#25); permanecem gitleaks/CodeQL, `schedule` de vencimentos e schema/OpenAPI fora do CI — Relatório Técnico §1 (Adendo) e §4, Risco-005 |
...
**Duas notas sobre o que os checks novos não são.** O CodeQL **informa**: o `analyze` falha por erro de varredura, não por achado, e alerta aberto vai para a aba Security com o check verde. Marcá-lo como *required* prometeria uma barreira que ele não é — e prometer barreira inexistente é o Risco-032 com outro nome. A política de dependências **não roda em `schedule`**: advisory publicado hoje deixaria a `main` vermelha sem ninguém ter tocado em nada, que é a polaridade que o relógio resolveu.
