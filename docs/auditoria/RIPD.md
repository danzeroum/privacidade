...
> como registro. A coluna **Fechado por** é o adendo de 2026-07-30 e diz onde ela deixou de valer: 18 riscos
> fechados, 0 parciais, 22 como descritos. Método e limites do adendo no §7.5.
...
| Risco-005 | Governança de mudança rompida: upload manual reverteu PR revisado, a main estava vermelha e nenhum workflow era required status check | P1 | Lacuna real | **Fechado** | `2cf1c18` `f86020e` `e1a35df` `77afe42` · sub-item (d) fechado: schema e invariantes entram no CI pelo check `Constraints e invariantes do banco`, e o contrato passa a ser lido pelo teste `PR 16 · invariante de paridade — contrato e mock, nos dois sentidos (Risco-034)`. Sub-item (c) fechado em `e1a35df`: workflow `Relógio do programa` roda diariamente as quatro varreduras de prazo e abre alerta idempotente, provado por `PR 29 · Risco-005(c) — o relógio do programa dispara sem sessão aberta`. Sub-item (b) fechado em `77afe42`: três jobs nomeados em `Segurança` — `Segredos no diff e no histórico` (gitleaks sobre a árvore e o histórico completo), `Dependências vulneráveis` (limiar `high` com exceção datada) e `CodeQL`. Provado por `PR 30 · Risco-005(b) — segredo, dependência e código sob varredura`. **Nota:** o CodeQL reprova **alerta novo**, e só ele — o `analyze` falha por erro de execução, e é um segundo check homônimo, do code scanning, que reprova quando o diff acrescenta alerta. Alerta pré-existente aparece na aba Security sem bloquear ninguém | Main restaurada (#19), T11 entregue (#18), proteção de branch evidenciada (#21–#25); permanecem gitleaks/CodeQL, `schedule` de vencimentos e schema/OpenAPI fora do CI — Relatório Técnico §1 (Adendo) e §4, Risco-005 |
...
| Risco-008 | Fornecedor não é entidade: DPA sem validação programática, expiração sem vigilância, sem chave por parceiro nem revogação com SLA | P1 | Lacuna real | **Fechado** | `4aa1f78` `c081ce8` `55bb029` · testes `PR 21 · unidade — a vigência do DPA, na fronteira de um dia`, `PR 21 · integração — a varredura pega o que o trigger não alcança` e `PR 31 · Risco-008 (resíduo) — desligar parceiro destruindo a chave dele`. Entidade, DPA no banco e vigilância de expiração no primeiro; a revogação com SLA nos dois últimos: o fornecedor entra na máquina de estados como nono artefato (`ativo → desligando → desligado`), o banco recusa transferência para quem está saindo antes de a chave cair, a destruição só é aceita depois da janela do DPA e gera prova encadeada, e a janela vencida sem destruição vira pendência `DESLIG-*` do relógio. **Resíduo declarado:** a chave é registro versionado, não material criptográfico em KMS — destruir aqui é apagar a referência, não zerar HSM | Recomendação definida no Relatório Técnico §4, Risco-008 |
...
| Risco-012 | GET /titulares/{id} devolve a ficha do titular sem finalidade e sem gravar no trail | P1 | Divergência | **Fechado** | Fechado — `238d018`. Provas: `PR 36 · Risco-012 — a ficha do balcão, servida e registrada`; e a catraca comportamental `PR 23 · sistema — a catraca comportamental do Risco-011`, que já esperava a ação `TITULAR_CONSULTADO` e passou a cobri-la sem precisar mudar. A rota estava no contrato e a política já exigia finalidade e ação própria — o que faltava era **servi-la**: caía no 404 genérico, e a Regra 3 só alcança rota servida. Agora grava antes de procurar (falha de log → 503 e nenhum valor sai), a sondagem que não achou nada também entra no trail, e o 404 de inexistente é byte a byte o de fora de escopo. Sai `mascara`, nunca `segredos`: o campo sensível aparece sem valor e sem caminho, e a revelação continua só por `POST /pseudonyms/resolve`. **Decisão declarada:** `TITULAR_CONSULTADO` saiu de `ACOES_PII` — a ficha devolve máscara, e cobrar justificativa de 20 caracteres a cada atendimento produziria prosa de fachada, o mesmo raciocínio que manteve `TITULAR_BUSCADO` fora da lista; a revelação mantém as duas exigências. | Recomendação definida no Relatório Técnico §4, Risco-012 |
...
4. **A catraca só vale onde ela roda.** Até `4778663` o CI era Ubuntu, e a condição de não-produção do Risco-035 dependia de um script que, num agente Windows, não chegava a executar: `VITE_PERFIL=producao vite build` é sintaxe de `sh`, e o `cmd.exe` lê `VITE_PERFIL=producao` como o nome do programa. O artefato saía com o perfil de demonstração, ou não saía — e a suíte, rodando só em Ubuntu, declarava verde. A **matriz `ubuntu-latest` + `windows-latest`** é o instrumento que cobre essa classe: defeito de plataforma não é achado por leitura de diff, é achado por execução no ambiente onde ele existe. Os quatro jobs Node puros rodam nas duas, com os legs Windows publicados como checks próprios (`… (windows)`); os quatro que ficam de fora declaram o motivo no próprio YAML, e um teste cobra que todo job faça uma coisa ou a outra. Medido no runner Windows: `core.autocrlf` chega em `true`, e é o `.gitattributes` — não o acaso — que mantém LF na árvore.
...
**O Risco-005 fechou inteiro em `77afe42`, e levou quatro commits para isso** — `2cf1c18` (schema no CI), `f86020e` (contrato sob catraca de leitura), `e1a35df` (o relógio) e `77afe42` (segredo, dependência e código). Um risco com quatro sub-itens não fecha por um deles, e a coluna citou os quatro desde a primeira anotação.

**Duas notas sobre o alcance dos checks novos, e a primeira eu escrevi errada antes de o próprio CodeQL me corrigir.** Ele reprova **alerta novo**: o `analyze` do workflow falha só por erro de execução, e o GitHub cria um segundo check homônimo, do code scanning, que reprova quando o diff **acrescenta** alerta. Alerta que já existia aparece na aba Security e não bloqueia. A versão anterior desta nota dizia "informa e não bloqueia" — e o PR que a introduziu ficou vermelho por dois alertas `high` de path injection no CLI de auditoria, contra a própria frase. Corrigido no código, e não dispensado à mão: o caminho saiu do argumento e o relatório passou a entrar por stdin. A política de dependências **não roda em `schedule`**: advisory publicado hoje deixaria a `main` vermelha sem ninguém ter tocado em nada, que é a polaridade que o relógio resolveu.

**Resíduo do fechamento:** dois advisories `moderate` em `react-router` e `react-router-dom`, abaixo do limiar e com correção disponível; e a PII sintética anterior ao `3fd6bf7`, que o gitleaks não vê — não é segredo — e que reescrever histórico resolveria, decisão de quem opera o repositório e não deste documento.
